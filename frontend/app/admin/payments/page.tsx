...
import {
  ...
  Loader2, Plus, CheckCircle, X, TrendingUp, DollarSign, Shield
} from 'lucide-react'
...
export default function AdminPaymentsPage() {
  ...
  const stats = React.useMemo(() => {
    let total = 0, rent = 0, deposits = 0, other = 0
    payments.forEach(p => {
      const amt = parseFloat(p.amount || 0)
      total += amt
      if (p.type === 'monthly') rent += amt
      else if (p.type === 'security_deposit') deposits += amt
      else other += amt
    })
    return { total, rent, deposits, other }
  }, [payments])
  ...
  const filteredPayments = payments.filter(pay => {
    ...
    return (name.includes(term) || id.includes(term) || txn.includes(term)) &&
      (typeFilter === 'all' || pay.type === typeFilter)
  })
  ...
  const TYPE_BADGES: Record<string, { label: string; bg: string; color: string }> = {
    ...
    other: { label: 'Other', bg: 'rgba(127,140,141,0.1)', color: '#7F8C8D' }
  }

  const inputCls = 'w-full px-3 py-2.5 rounded-xl text-sm border border-white/8 bg-[#0A0E1A] text-white outline-none focus:border-amber-500'
      ...
      >
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-5">
          <div className="pl-12 sm:pl-0">
            ...
            <p className="text-xs text-gray-500 mt-1">Audit log of all student financial collections and deposit clearances</p>
          </div>
          <button
            onClick={() => setManualModalOpen(true)}
            className="self-start sm:self-auto flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-bold transition-all whitespace-nowrap"
        ...
        {/* Stats Row — 4 pills, compact */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-5">
          {[
            { label: 'Gross Collections', val: stats.total, icon: TrendingUp, color: '#27AE60', bg: 'rgba(46,204,113,0.06)' },
            { label: 'Rent Collected', val: stats.rent, icon: DollarSign, color: '#3B82F6', bg: 'rgba(59,130,246,0.06)' },
            { label: 'Deposits Received', val: stats.deposits, icon: Shield, color: '#9B59B6', bg: 'rgba(155,89,182,0.06)' },
            { label: 'Other Collections', val: stats.other, icon: ArrowUpRight, color: '#F59E0B', bg: 'rgba(245,158,11,0.06)' },
          ].map((s, i) => {
            const Icon = s.icon
            return (
              <div key={i} className="p-4 rounded-2xl border border-white/5 flex items-center justify-between" style={{ background: '#0F1629' }}>
                <div>
                  <p className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider">{s.label}</p>
                  <p className="text-lg sm:text-xl font-bold text-white mt-0.5">₹{s.val.toLocaleString('en-IN')}</p>
                </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center mb-5">
          <div className="admin-filter-bar">
            {(['all', 'monthly', 'security_deposit', 'renewal', 'other'] as const).map(f => (
              <button key={f} onClick={() => setTypeFilter(f)} className={typeFilter === f ? 'active' : ''}>
                {f === 'all' ? 'All' : f === 'monthly' ? 'Rent' : f === 'security_deposit' ? 'Deposits' : f === 'renewal' ? 'Renewals' : 'Others'}
              </button>
            ))}
          </div>
          <div className="w-full sm:w-80 relative">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" />
            <input type="text" placeholder="Search by student name or Transaction ID…"
        ...
        {loading ? (
          ...
          <div className="admin-empty-compact">
            ...
            <p className="text-sm font-semibold text-gray-300">No payment logs found</p>
            <p className="text-xs text-gray-500">Record a manual payment using the button above</p>
          </div>

        ) : (
          <>
            {/* Desktop Table */}
            <div className="admin-table-wrapper bg-[#0F1629] border border-white/5 rounded-2xl overflow-hidden shadow-xl">
                ...
                <table className="w-full min-w-[800px] table-auto text-left border-collapse text-sm">
                  <thead>
                    <tr className="bg-white/5 border-b border-white/5 text-gray-400">
                      {['Student', 'Txn Type', 'Mode', 'Amount', 'Transaction ID', 'Date Recorded', 'Receipt'].map(h => (
                        <th key={h} className={`px-5 py-3.5 text-xs font-bold uppercase tracking-wider ${h === 'Receipt' ? 'text-right' : ''}`}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                    ...
                    {filteredPayments.map((pay) => {
                      ...
                      return (
                        <tr key={pay.id} className="hover:bg-white/[0.02] transition-colors">
                          ...
                          <td className="px-5 py-3.5 whitespace-nowrap text-xs text-gray-300 uppercase">{pay.payment_mode || '—'}</td>
                          <td className="px-5 py-3.5 whitespace-nowrap font-bold text-white">₹{pay.amount}</td>
                          <td className="px-5 py-3.5 font-mono text-xs text-gray-400 whitespace-nowrap">{pay.transaction_id || 'N/A'}</td>
            ...
            <div className="admin-mobile-card space-y-3">
              {filteredPayments.map((pay) => {
                const badge = TYPE_BADGES[pay.type] || TYPE_BADGES.other
                return (
                  <div key={pay.id} className="bg-[#0F1629] border border-white/8 rounded-2xl p-4">
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <p className="font-bold text-white text-sm">{pay.students?.full_name}</p>
                        <p className="text-xs text-gray-500 font-mono">{pay.students?.student_id || '—'}</p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-gray-400 mb-3">
                      <div><span className="text-gray-600">Amount:</span> <span className="text-white font-bold">₹{pay.amount}</span></div>
                      <div><span className="text-gray-600">Mode:</span> {pay.payment_mode || '—'}</div>
                      <div><span className="text-gray-600">Date:</span> {pay.created_at?.split('T')[0]}</div>
                      <div className="truncate"><span className="text-gray-600">TxnID:</span> {pay.transaction_id || 'N/A'}</div>
                    </div>
                  </div>
                )
              })}
              ...
              <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }}
                ...
                <div className="bg-[#0F1629] border border-white/10 rounded-2xl w-full max-w-md shadow-2xl text-gray-200 overflow-y-auto max-h-[90vh]">
                  <div className="flex items-center justify-between p-6 border-b border-white/5">
                    <h3 className="text-lg font-bold" style={{ fontFamily: 'Playfair Display' }}>Record Manual Payment</h3>
                  ...
                  <form onSubmit={handleManualPayment} className="p-6 space-y-4">
                    <div>
                      ...
                      <select required value={manualForm.student_id}
                        onChange={e => setManualForm(p => ({ ...p, student_id: e.target.value }))}
                        className={inputCls}>
                        <option value="">Select student…</option>
                        {students.map(s => (
                          <option key={s.id} value={s.id}>{s.full_name} ({s.student_id || s.email})</option>
                        ))}
                      </select>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="number" required min="1" placeholder="e.g. 9500"
                      ...
                      <div>
                        ...
                        <input type="date" required value={manualForm.paid_date}
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div>
                        ...
                        <select value={manualForm.payment_mode}
                    ...
                    <div>
                      ...
                      <input type="text" placeholder="UPI Ref / Txn ID / Cheque No."
                        value={manualForm.transaction_id} onChange={e => setManualForm(p => ({ ...p, transaction_id: e.target.value }))}
                        className={inputCls} />
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                      ...
                      <button type="submit" disabled={submittingManual}
                        ...
                        {submittingManual ? <><Loader2 size={15} className="animate-spin" /> Saving…</> : <><CheckCircle size={15} /> Record Payment</>}
                      </button>
                    </div>
                  </form>
                </div>
